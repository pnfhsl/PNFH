...
class rolesModel extends database
{
	...
	public function validarConsultar($metodo, $data = "")
	{
		...
		if ($metodo == "ConsultarAccesosModulo") {
			$result = self::ConsultarAccesosModulo($data);
			return $result;
		}
		if ($metodo == "getOne") {
	...
	private function ConsultarAccesosModulo($data)
	{
		try {
			$id_rol = $data['id_rol'];
			$nombre_modulo = $data['nombre_modulo'];
			$query = parent::prepare("SELECT * FROM accesos, roles, permisos, modulos WHERE roles.id_rol = accesos.id_rol and permisos.id_permiso = accesos.id_permiso and modulos.id_modulo = accesos.id_modulo and roles.estatus = 1 and permisos.estatus = 1 and modulos.estatus = 1 and accesos.estatus = 1 and accesos.id_rol = :id_rol and modulos.nombre_modulo = :nombre_modulo");
			$query->bindValue(":id_rol",$id_rol);
			$query->bindValue(":nombre_modulo",$nombre_modulo);
			$respuestaArreglo = '';
			$query->execute();
			$query->setFetchMode(parent::FETCH_ASSOC);
			$respuestaArreglo = $query->fetchAll(parent::FETCH_ASSOC);
