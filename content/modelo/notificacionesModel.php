...
class notificacionesModel extends database
{
	...
	public function __construct()
	{
		...
		$this->fechaDesde = date('Y-m-d', time() - ($unMes + $unMes) );
		$this->fechaActual = date('Y-m-d');
		$this->fechaHasta = date('Y-m-d', time() + $unMes );

	}
	public function ConsultarNotificacionClasesListas($data, $rol)
	{
		try {
			$query = parent::prepare("SELECT * FROM notificaciones WHERE notificaciones.estatus = 1 and notificaciones.fecha_notificacion BETWEEN '{$data['inicio']}' and '{$data['final']}' and notificaciones.tabla_notificacion = 'clases' and notificaciones.visto_superusuario < 5 and notificaciones.estatus = 1");
			$respuestaArreglo = '';
			$query->execute();
	...
	{
		try {
			$sql = "SELECT * FROM periodos, secciones, clases, saberes, profesores, notificaciones WHERE notificaciones.id_tabla = clases.id_clase and notificaciones.id_tabla = {$data['id_tabla']} and periodos.estatus = 1 and secciones.estatus = 1 and clases.estatus = 1 and saberes.estatus = 1 and periodos.id_periodo = clases.id_periodo and clases.cedula_profesor = profesores.cedula_profesor and saberes.id_SC = clases.id_SC and secciones.cod_seccion = clases.cod_seccion and notificaciones.estatus = 1";
			$sql .= " and periodos.fecha_cierre BETWEEN '{$this->fechaDesde}' and '{$this->fechaHasta}'";
	...
	public function BuscarClasesPorNotificacionProfesor($data, $cedula)
	{
		try {
			...
			$sql = "SELECT * FROM periodos, secciones, clases, saberes, profesores, notificaciones WHERE {$elemento_tabla} = clases.id_clase and {$elemento_tabla} = {$id_tabla} and periodos.estatus = 1 and secciones.estatus = 1 and clases.estatus = 1 and saberes.estatus = 1 and periodos.id_periodo = clases.id_periodo and saberes.id_SC = clases.id_SC and secciones.cod_seccion = clases.cod_seccion and clases.cedula_profesor = profesores.cedula_profesor and clases.cedula_profesor = '{$cedula}' and notificaciones.estatus = 1";
			$sql .= " and periodos.fecha_cierre BETWEEN '{$this->fechaDesde}' and '{$this->fechaHasta}'";
	...
	public function BuscarClasesPorNotificacionAlumno($data, $cedula, $notas = false)
	{
		try {
			if ($notas == false) {
				// $sql = "SELECT * FROM profesores, proyectos, grupos, seccion_alumno, alumnos, notas, clases, saberes, secciones, periodos, notificaciones WHERE profesores.cedula_profesor = proyectos.cedula_profesor and proyectos.cod_proyecto = grupos.cod_proyecto and grupos.id_SA = seccion_alumno.id_SA and alumnos.cedula_alumno = seccion_alumno.cedula_alumno and seccion_alumno.id_SA = notas.id_SA and notas.id_clase = clases.id_clase and clases.id_SC = saberes.id_SC and clases.cod_seccion = secciones.cod_seccion and clases.id_periodo = periodos.id_periodo and notificaciones.id_tabla = clases.id_clase and notificaciones.id_tabla = {$data['id_tabla']} and seccion_alumno.cedula_alumno = '{$cedula}'  and notificaciones.estatus = 1";

				$sql = "SELECT * FROM notificaciones, profesores, clases, secciones, periodos, seccion_alumno, alumnos, notas, saberes WHERE notificaciones.tabla_notificacion = 'clases' and notificaciones.id_tabla = clases.id_clase and profesores.cedula_profesor = clases.cedula_profesor and clases.id_SC = saberes.id_SC and clases.cod_seccion = secciones.cod_seccion and clases.id_periodo = periodos.id_periodo and secciones.cod_seccion = seccion_alumno.cod_seccion and seccion_alumno.cedula_alumno = alumnos.cedula_alumno and clases.id_clase = notas.id_clase and notas.id_SA = seccion_alumno.id_SA and clases.estatus = 1 and secciones.estatus = 1 and notas.estatus = 1 and saberes.estatus = 1 and clases.id_clase = {$data['id_tabla']} and seccion_alumno.cedula_alumno = '{$cedula}'";
				// $sql .= " and periodos.fecha_cierre BETWEEN '{$this->fechaDesde}' and '{$this->fechaHasta}'";

			} else {
				$sql = "SELECT * FROM notificaciones, notas, seccion_alumno, clases WHERE notificaciones.codigo_tabla = notas.id_nota and notas.id_SA = seccion_alumno.id_SA and clases.id_clase = notas.id_clase  and seccion_alumno.cedula_alumno = '{$cedula}' and clases.id_clase = {$data['id_clase']}  and notificaciones.estatus = 1";
			}
			$query = parent::prepare($sql);
			$respuestaArreglo = '';
			$query->execute();
	...
	{
		try {
			$query = parent::prepare("SELECT * FROM periodos, secciones, clases, saberes WHERE periodos.estatus = 1 and secciones.estatus = 1 and clases.estatus = 1 and saberes.estatus = 1 and periodos.id_periodo = clases.id_periodo and saberes.id_SC = clases.id_SC and secciones.cod_seccion = clases.cod_seccion and clases.id_periodo = {$data['id_periodo']}");
			$respuestaArreglo = '';
			$query->execute();
			$query->setFetchMode(parent::FETCH_ASSOC);
			$respuestaArreglo = $query->fetchAll(parent::FETCH_ASSOC);
	...
	public function Eliminar($data){
		...
		try {
			if(mb_strtolower($data['tabla'])=="notas"){
				// $query = parent::prepare("DELETE FROM notificaciones WHERE tabla_notificacion = :tabla and codigo_tabla = :codigo");
				$query = parent::prepare("UPDATE notificaciones SET estatus = 0 WHERE tabla_notificacion = :tabla and codigo_tabla = :codigo");
				$query->bindValue(':tabla', $data['tabla']);
				$query->bindValue(':codigo', $data['codigo']);
			}
			if(mb_strtolower($data['tabla'])=="clases"){
				$query = parent::prepare("UPDATE notificaciones SET estatus = 0 WHERE tabla_notificacion = :tabla and id_tabla = :id");
				// $query = parent::prepare("DELETE FROM notificaciones WHERE tabla_notificacion = :tabla and id_tabla = :id");
				$query->bindValue(':tabla', $data['tabla']);
