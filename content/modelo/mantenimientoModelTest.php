...
class mantenimientoModel extends database
{
	...
	public function __construct()
	{
		...
		$this->mysqlImportFilename = 'libs/backup/' . 'higiene' . "_" . $this->fecha . '.sql';
	}
	...
	public function Respaldar($fechaAct = "", $tables = '*')
	{
		...
		$return = '';
		$link = new mysqli('localhost', 'pnfh', 'pnf.higiene', 'higiene');
		if ($tables == '*') {
			$tables = array();
		...
		try {
			if ($fechaAct == "") {
				...
				$this->mysqlImportFilename = 'libs/backup/automatico/' . 'higiene' . "_" . $fechaAct . '.sql';
			}
			...
			$response = './libs/backup/' . 'higiene' . "_" . $this->fecha . '.sql';
			// $response = '/backup/'.'higiene'."_".$this->fecha.'.sql';
	...
	{
		$cnn = new mysqli('localhost', 'pnfhsl', '#pnf.Higiene', 'higiene');
		$this->mysqlRestoreFilename = $file;
		$sql = "";
		$error = "";

		$result = mysqli_query($cnn, "DROP DATABASE " . 'higiene');
		if ($result) {
			$result = mysqli_query($cnn, "CREATE DATABASE " . 'higiene');
			// echo "  \n\n CREAR BD ".$result."  \n\n ";
			if ($result) {
				$cnn = new mysqli('localhost', 'pnfhsl', '#pnf.Higiene', 'higiene');
